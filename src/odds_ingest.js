...
function matchGame(games, row) {
  ...
  for (const g of games) {
    ...
    if (!gh || !ga) continue;
    if (same(gh, h) && same(ga, a)) return { game: g, flipped: false };
    if (same(gh, a) && same(ga, h)) return { game: g, flipped: true };
  }
  return null;
}
  ...
  for (const row of rows) {
    ...
    if (!bySport.has(sport)) {
      bySport.set(sport, db.prepare(
        `SELECT espn_game_id, home_team, away_team FROM today_games WHERE sport = ?`
      ).all(sport));
    }
    ...
    const spH = numOrNull(f ? row.spread_away : row.spread_home);
    const spA = numOrNull(f ? row.spread_home : row.spread_away);
    stmt.run(
...
module.exports = { storeEngineBookLines };
