...
function emptyRow(book, sport, home, away) {
  return {
...
function kambiParseEvent(sport, e) {
  ...
  const row = emptyRow('betrivers', sport, ev.homeName, ev.awayName);
  ...
  for (const [sport, leagueId] of Object.entries(PINNACLE_LEAGUES)) {
    try {
      ...
      for (const m of Array.isArray(matchups) ? matchups : []) {
        ...
        games.set(m.id, emptyRow('pinnacle', sport, home.name, away.name));
      }
    ...
    try {
      ...
      for (const mk of markets) {
        ...
        if (isML || isSpread) {
          ...
          if (!home || !away) continue;
          if (!row) { row = emptyRow('fanduel', sport, home.runnerName, away.runnerName); byEvent.set(mk.eventId, row); }
          if (isML) {
...
function dkNashParse(sport, data) {
  ...
  for (const ev of data.events || []) {
    ...
    if (!home || !away) continue;
    events.set(ev.id, emptyRow('draftkings', sport, home.name, away.name));
  }
  ...
  for (const [sport, leagueId] of Object.entries(DK_NASH_LEAGUES)) {
    try {
      const data = await getJson(
        ...
        { Referer: 'https://sportsbook.draftkings.com/', Origin: 'https://sportsbook.draftkings.com' }
      );
